...
emb = np.load(
    "/home/patchethium/PycharmProjects/Clio-training/ckpt/PhonemeEmbedding.npy",
    allow_pickle=True,
).tolist()
...
def main():
    ...
    (phoneme_list, dur_list, pit_list, eng_list, pos_list, str_list) = [
        np.load(path.join(h["variance_path"], x), allow_pickle=True)
        for x in (
            "phoneme.npy",
            "duration.npy",
            "pitch.npy",
    ...
    for idx in tqdm(range(len(phoneme_list))):
        ...
        p_2 = torch.cat(
            (p_1, torch.from_numpy(str_list[idx])), 1
        )  # [S,N], N = 6; this is the input of variance predictor

        p_3 = torch.cat(
            (p_2, torch.from_numpy(pit_list[idx]).unsqueeze(1)), 1
        )  # [S,N], N = 7
        p_4 = torch.cat(
            (p_3, torch.from_numpy(eng_list[idx]).unsqueeze(1)), 1
        )  # [S,N], N = 8
    ...
    np.save(
        path.join(h["output_path"], "vanilla_phoneme.npy"),
        np.array(vanilla_phoneme_list, dtype="object"),
        allow_pickle=True,
    )
    np.save(
        path.join(h["output_path"], "variance_phoneme.npy"),
        np.array(variance_phoneme_list, dtype="object"),
        allow_pickle=True,
    )

    print(f"Done. larger: {larger}, less: {less}")
    print(
        f"average larger sum: {larger_sum / larger}, average less sum: {less_sum / less}"
    )
