<script lang="ts">
  ...
  function handlePhonemeInput(e: any, i: number, j: number) {
    ...
    if (newPhoneme == "") {
      if (audioStore[i].marks.length == 1) {
        audioStore.splice(i, 1);
      } else {
...
<main>
  ...
  <div class="parent">
    <div class="div1">
      <RadioButton bind:group={curPanel} value="pit" style="margin: 0 0 0 30px"
        >Pitch</RadioButton
      >
      <RadioButton bind:group={curPanel} value="dur" style="margin: 0 0 0 15px"
        >Duration</RadioButton
      >
      <RadioButton bind:group={curPanel} value="eng" style="margin: 0 0 0 15px"
        >Energy</RadioButton
      >
    </div>
    <div class="sliders">
      <Slider
        ...
        step={0.01}
        tooltip={false}
        bind:value={pit_shift}
        on:wheel={(event) =>
          (pit_shift += 0.01 * -Math.sign(event.deltaY) * shiftKeyFlag)}
      />
      ...
      <Slider
        ...
        step={0.01}
        tooltip={false}
        bind:value={dur_scale}
        on:wheel={(event) =>
          (dur_scale += 0.01 * -Math.sign(event.deltaY) * shiftKeyFlag)}
      />
      ...
      <Slider
        ...
        step={0.01}
        tooltip={false}
        bind:value={eng_shift}
        on:wheel={(event) =>
          (eng_shift += 0.01 * -Math.sign(event.deltaY) * shiftKeyFlag)}
      />
      ...
      {#each audioStore as audioItem, i}
        {#each audioItem.marks as marks, j}
          <div style="display: grid">
            <div
              style="block-size: 120px; margin-left:12px; margin-right: 12px"
            >
              {#if curPanel === "pit"}
                ...
                <Slider
                  ...
                  on:wheel={(event) =>
                    (marks.pit +=
                      0.01 * -Math.sign(event.deltaY) * shiftKeyFlag)}
              ...
              <div>
                <Flyout placement="right">
                  ...
                  <svelte:fragment slot="flyout">
                    <input
                      value={marks.mark +
                        (marks.stress == null ? "" : marks.stress)}
                      on:change={(e) => handlePhonemeInput(e, i, j)}
                    />
                  </svelte:fragment>
                </Flyout>
              </div>
